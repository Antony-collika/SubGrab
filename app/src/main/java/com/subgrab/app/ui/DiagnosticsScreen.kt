...
@Composable fun DiagnosticsScreen(database:SubGrabDatabase,onBack:()->Unit){
 val context=LocalContext.current;val scope=rememberCoroutineScope();var metrics by remember{mutableStateOf<List<RequestMetricEntity>>(emptyList())};var logs by remember{mutableStateOf<List<RuntimeLogEntity>>(emptyList())}
 ...
 Column(Modifier.fillMaxSize().padding(16.dp),verticalArrangement=Arrangement.spacedBy(10.dp)){
  ...
  OutlinedButton(onClick={
   val text=buildString{
    appendLine("Governor")
    RequestLane.entries.forEach { lane->appendLine("${lane.name}: state=${RequestGovernor.runtime().state(lane)}, delay=${RequestGovernor.runtime().delay(lane)}ms") }
    appendLine("Request metrics")
    RequestLane.entries.forEach { lane->val rows=metrics.filter{it.lane==lane.name};val d=rows.map{it.durationMs}.sorted();val benign=rows.count{it.failureType in setOf("NO_SUBTITLE","LANGUAGE_UNAVAILABLE","VIDEO_UNAVAILABLE")};val negative=rows.count{!it.success&&it.failureType !in setOf("NO_SUBTITLE","LANGUAGE_UNAVAILABLE","VIDEO_UNAVAILABLE")};appendLine("${lane.name}: total=${rows.size}, success=${rows.count{it.success}}, benign=$benign, negative=$negative, avg=${if(d.isEmpty())0 else d.average().toLong()}ms, p50=${percentile(d,.50)}ms, p95=${percentile(d,.95)}ms") }
    appendLine("Runtime log")
    logs.forEach { log->appendLine("${java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss",java.util.Locale.US).format(java.util.Date(log.timestamp))} [${log.category}] ${log.message}") }
   }
   val clipboard=context.getSystemService(Context.CLIPBOARD_SERVICE) as ClipboardManager
   clipboard.setPrimaryClip(ClipData.newPlainText("SubGrab diagnostics",text))
  },modifier=Modifier.fillMaxWidth()){Text("Sao chép diagnostics")}
