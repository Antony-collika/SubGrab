...
class YouTubeDataApiClient(private val settings:SettingsRepository,private val pacer:RequestPacer){
 ...
 suspend fun listPlaylistItems(source:String):List<VideoItem>{
  ...
  val a=json.optJSONArray("items")?:return emptyList();val ids=buildList{for(i in 0 until a.length()){a.optJSONObject(i)?.optJSONObject("snippet")?.optString("resourceId")?.let{}}}
  val videoIds=buildList{for(i in 0 until a.length()){a.optJSONObject(i)?.optJSONObject("snippet")?.optJSONObject("resourceId")?.optString("videoId")?.takeIf{it.isNotBlank()}?.let(::add)}}
