...
class RequestPacer(private val settings:SettingsRepository,private val governor:RequestGovernor,private val database:SubGrabDatabase){
 suspend fun <T> execute(operation:RequestOperation,block:suspend()->T):T {
  ...
  database.runtimeLogDao().insert(RuntimeLogEntity(timestamp=System.currentTimeMillis(),level="INFO",category="REQUEST",lane=operation.lane.name,operation=operation.operation,message="request start"))\n  val started=System.currentTimeMillis(); var status:Int?=null
  try { val value=block(); val result=RequestResult(true,status,System.currentTimeMillis()-started,null); persist(operation,result);val oldDelay=governor.delay(operation.lane);val changed=governor.observe(operation.lane,result);if(changed) logGovernor(operation,oldDelay);return value }
  catch(t:Throwable){status=(t as? HttpFailure)?.status;val failureType=(t as? SubtitleFailure)?.type ?: FailureClassifier.classify(status,t);val result=RequestResult(false,status,System.currentTimeMillis()-started,failureType);persist(operation,result);val oldDelay=governor.delay(operation.lane);val changed=governor.observe(operation.lane,result);if(changed) logGovernor(operation,oldDelay);throw t}
 }
 private suspend fun logGovernor(op:RequestOperation,oldDelay:Long){database.runtimeLogDao().insert(RuntimeLogEntity(timestamp=System.currentTimeMillis(),level="INFO",category="GOVERNOR",lane=op.lane.name,operation=op.operation,message="state="+governor.state(op.lane)+" delay="+governor.delay(op.lane)+" previousDelay="+oldDelay))}\n private suspend fun persist(op:RequestOperation,r:RequestResult){
