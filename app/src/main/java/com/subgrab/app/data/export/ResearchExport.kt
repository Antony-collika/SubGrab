...
object ResearchExport {
    ...
    fun csv(results: List<VideoSearchResult>): String {
        ...
        return buildString {
            ...
            results.forEach { v ->
                appendLine(listOf(v.videoId,v.title,v.channelName,v.publishedAt,v.fetchedAt,v.durationSeconds,v.subscriberCount,v.viewCount,v.likeCount,v.commentCount,v.category,v.tags,v.topic,v.playlistTitles,v.searchKeywords).joinToString(",") { escape(it?.toString().orEmpty()) })
            }
        }
    }
    ...
    fun markdown(results: List<VideoSearchResult>): String = buildString {
        ...
        appendLine("|---|---|---:|---:|---|---|")
        results.forEach { v ->
            appendLine("| ${cell(v.title)} | ${cell(v.channelName)} | ${v.viewCount ?: 0} | ${v.likeCount ?: 0} | ${cell(v.publishedAt)} | `${cell(v.videoId)}` |")
        }
    }

    private fun escape(value: String): String = """ + value.replace(""", """").replace("\n", " ").replace("\r", " ") + """
    private fun cell(value: String?): String = value.orEmpty().replace("|", "\|").replace("\n", " ").replace("\r", " ")
}
