...
class ResearchRepository(private val database: SubGrabDatabase) {
    ...
    suspend fun search(query: String, filters: ResearchFilters = ResearchFilters(),
        ...
        if (query.trim().isNotBlank()) {
            conditions += "(m.videoId IN (SELECT videoId FROM video_search WHERE video_search.video_search MATCH ?) OR EXISTS (SELECT 1 FROM playlist_video pvq JOIN playlists pq ON pq.playlistId = pvq.playlistId WHERE pvq.videoId = m.videoId AND LOWER(pq.title) LIKE LOWER(?)))"
            args += ftsQuery(query)
            args += "%" + query.trim() + "%"
        }
