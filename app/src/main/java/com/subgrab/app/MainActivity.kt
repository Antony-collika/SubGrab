...
@Composable fun SubGrabApp() {
    val context = LocalContext.current; val factory = remember(context) { DownloadViewModelFactory(context) }; val vm: DownloadViewModel = viewModel(factory = factory); val state by vm.state.collectAsState(); val settingsRepo = remember(context) { SettingsRepository(context) }; val settings by settingsRepo.settings.collectAsState(initial = AppSettings()); var showSettings by remember { mutableStateOf(false) }
    Scaffold(topBar = { TopAppBar(title = { Text("SubGrab") }, actions = { IconButton(onClick = { showSettings = true }) { Icon(Icons.Default.Settings, "Cài đặt") } }) }) { pad ->
        if (showSettings) SettingsScreen(settingsRepo) { showSettings = false } else when (val current = state) { is AnalysisState.Ready -> SelectVideoScreen(current.videos, current.folder, vm, settings, Modifier.padding(pad)); else -> HomeScreen(state, vm, Modifier.padding(pad)) }
    }
}

@Composable private fun HomeScreen(state: AnalysisState, vm: DownloadViewModel, modifier: Modifier) { val context = LocalContext.current; var url by remember { mutableStateOf("") }; LazyColumn(modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(16.dp)) {
...
@Composable private fun SelectVideoScreen(videos: List<VideoItem>, folder: String, vm: DownloadViewModel, settings: AppSettings, modifier: Modifier) { val context = LocalContext.current; var folderName by remember(folder) { mutableStateOf(folder) }; var started by remember { mutableStateOf(false) }; val selected = videos.count { it.isSelected }; Column(modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
    ...
    Button(onClick = { ContextCompat.startForegroundService(context, android.content.Intent(context, DownloadService::class.java)); vm.startDownload(settings); started = true }, enabled = selected > 0 && folderName.isNotBlank(), modifier = Modifier.fillMaxWidth()) { Text("TẢI PHỤ ĐỀ ($selected)") }
    if (started) Text("Đã khởi chạy tải tuần tự. Tiến độ hiển thị qua notification.", color = MaterialTheme.colorScheme.primary)
} }
